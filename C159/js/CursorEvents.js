...
AFRAME.registerComponent("cursor-listener", {
  ...
  handleClickEvents: function () {
    /*
      first, we need to check for clicks. 
      So we'll add an event listener which will listen to all the mouse click events. 
  
      Inside the listener function, using the document.querySelector() method, we'll get the places-container and store it in the places container variable.
  
      Using the get attribute, we'll get the state from the tour entity.
      If the state is places-list then using the get attribute we'll get the id.
      Create a placedId list with all places id in it.
      Then we check if the placesId list contains the id that we have, if it
      does, then we set the state to view and set the id to selectedCard variable.
    ...
    this.el.addEventListener("click", evt => {
      ...
      if (state === "places-list") {
        ...
        const placesId = [
          ...
          //in the Tour.js, if the state of "tour" is equal to "view" then the element becomes hidden
  ...
  /*
  We defined a schema having selectedItemId as the data and wrote a function,
handlePlaceListState(), which checked for the selected thumbnail’s id.
  */
